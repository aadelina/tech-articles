...
## 3.场景
- 在现代前端框架（如 Vue、React）中，我们都会用到模板引擎（Vue 模板、React），
- 在服务器端和客户端通用的的模板引擎Mustache、EJS等
- 新型模板引擎lit-html，基于 web components技术，兼容不好
模板引擎多样，在合适的场景使用合适的模板引擎，使得开发效率更高。
## 4.优缺点
模板引擎使用也需要权衡
- 优点：  
    ...
    ✅ 提高安全性，防止 XSS 攻击，主流模板默认对动态内容进行 HTML 转义，防止输入恶意代码被执行  

    ✅ 性能优化，多数模板引擎进行“预编译”和“缓存”，首次使用被编译的可执行函数，后续直接调用可执行函数，避免重复解析模板
